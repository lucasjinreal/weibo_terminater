...
def get_cookie_from_network(account_id, account_password):
    ...
    if os.path.exists(phantom_js_driver_file):
        try:
            logging.info('loading PhantomJS from {}'.format(phantom_js_driver_file))
            ...
            logging.info('opening weibo login page, this is first done for prepare for cookies. be patience to waite load '
                  'complete.')
            count_time()
            driver.find_element_by_xpath(
                '//input[@id="loginName"]').send_keys(account_id)
            driver.find_element_by_xpath(
            ...
            logging.info('account password: {}'.format(account_password))

            driver.find_element_by_xpath('//a[@id="loginAction"]').click()
        except InvalidElementStateException as e:
            logging.info(e)
            logging.info('error, account id {} is not valid, pass this account, you can edit it and then '
                  'update cookies. \n'
                  .format(account_id))
        ...
        try:
            ...
            if 'SSOLoginState' in cookie_string:
                logging.info('success get cookies!! \n {}'.format(cookie_string))
                if os.path.exists(COOKIES_SAVE_PATH):
                    ...
                    if cookies_dict[account_id] is not None:
                        ...
                        logging.info('successfully save cookies into {}. \n'.format(
                            COOKIES_SAVE_PATH))
                    else:
                        pass
                else:
        ...
        logging.info('PhantomJS driver does not found, will using FireFox as Driver.')
        try:
